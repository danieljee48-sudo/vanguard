...
async function loadSubscription(){
 ...
 try{
  ...
  el.innerHTML=`<div class="card"><div class="row"><div><strong>${esc(plans[sub.plan]||'Business')} plan</strong><small>${esc(status)}${sub.cancel_at_period_end?' · Cancels at period end':''}</small></div><span class="badge">${esc(sub.billing_interval||'month')}</span></div><div class="actions" style="margin-top:12px"><button class="btn secondary" id="cleanManageBilling">Manage billing</button></div><small style="display:block;margin-top:8px;color:#64748b">${esc(tone)} · Update payment details, invoices or cancellation in Stripe.</small></div>`;const manage=document.getElementById('cleanManageBilling');if(manage)manage.onclick=async()=>{manage.disabled=true;manage.textContent='Opening…';try{await window.VGCleanSubscription?.openBillingPortal()}catch(e){alert(e.message||'Unable to open billing portal');manage.disabled=false;manage.textContent='Manage billing'}};
